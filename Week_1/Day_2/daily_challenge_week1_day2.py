...
# Step 1: Convert matrix_string to a 2D list (matrix)
matrix = [list(row) for row in MATRIX_STR.strip().split('\n')]# I use split to separate the string into rows and then convert each row into a list of characters.

# Step 2: Iterate through columns
decoded_message = ""
for col in range(len(matrix[0])): # I iterate through the columns by using the length of the first row to determine how many columns there are.
    temp_string = ""
    for row in range(len(matrix)): # I iterate through the rows for each column.
        char = matrix[row][col] # I access the character at the current row and column.
        if char.isalpha(): # I check if the character is an alphabet letter.
            temp_string += char # If it is, I add it to the temporary string.
        else:
            temp_string += " " # If it is not, I add a space to the temporary string.
    decoded_message += temp_string.strip() + " " # I add the temporary string to the decoded message, stripping any leading or trailing spaces.
